...
export default function useProductForm({ mode = 'create', initialData = {} } = {}) {
    ...
    const submit = useCallback(async (modeArg = mode, id) => {
        try {
            ...
            if (modeArg === 'create') {
                ...
                toast({ type: 'success', message: res?.data?.message || 'Updated', duration: 4000, autoClose: true, placement: "top-center", animation: "top-bottom" });
                return res?.data?.data || res?.data;
            }
        } catch (err) {
    ...
    const remove = useCallback(async (id) => {
        try {
            ...
            toast({ type: 'success', message: 'Deleted', duration: 4000, autoClose: true, placement: "top-center", animation: "top-bottom" });
            return true;
        } catch (err) {
            console.error('delete error', err);
            toast({ type: 'error', message: err?.response?.data?.message || err.message || 'Failed to delete', duration: 4000, autoClose: true, placement: "top-center", animation: "top-bottom" });
            throw err;
        }
    }, [formData, toast]);
