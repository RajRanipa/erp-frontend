...
notify.success = (m) => notify('success', m);
notify.error = (m) => notify('error', m);
  useEffect(() => {
    return () => {
  ...
  // Validate current state and notify parent about validity and dirtiness
  useEffect(() => {
  ...
  const onSelectFile = async (f) => {
    ...
    try {
      ...
      notify.success('Logo uploaded. Click Save to persist.');
    } catch (err) {
      console.error(err);
