...
/**
 * DensityDialog - reusable dialog component to create or edit a Density
 * Props:
 *  - open: boolean
 *  - onClose: function
 *  - mode: 'create' | 'edit'
 ...
 */
export default function DensityDialog({
  open,
  onClose,
  mode = 'create',
  initialData = {},
  ...
  const handleSave = useCallback(async () => {
    try {
      ...
      if (mode === 'create') {
        ...
        toast({ type: 'success', message: res?.data?.message || 'Density created' });
        onSaved(doc);
      } else {
        // edit
        ...
        const doc = res?.data?.data || res?.data;
        toast({ type: 'success', message: res?.data?.message || 'Density updated' });
        onSaved(doc);
      }
      ...
      const msg = err?.response?.data?.message || 'Failed to save density';
  ...
  const handleDelete = useCallback(async () => {
    try {
      ...
      if (!confirm('Are you sure you want to delete this density?')) return;
      setDeleting(true);
      ...
      console.error('Density delete error', err);
  ...
  return (
     <Dialog
      open={open}
      title={mode === 'create' ? 'Add Density' : 'Edit Density'}
