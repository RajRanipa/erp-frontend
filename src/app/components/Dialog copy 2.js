...
const Dialog = memo(function Dialog({
  ...
  useEffect(() => {
    ...
    const raf = requestAnimationFrame(() => {
      ...
      const focusable = container ? Array.from(container.querySelectorAll(FOCUSABLE_QUERY)).filter(isVisible) : [];
      if (focusable.length > 0) focusable[0].focus({ preventScroll: true });
      else if (panelRef.current) panelRef.current.focus({ preventScroll: true });
    });
    return () => cancelAnimationFrame(raf);
  }, [open, children, openCounter.current]);
    ...
    <Portal>
      ...
      >
        ...
        >
          ...
          <div ref={childrenRef} className="p-4 space-y-3 overflow-auto h-fit">{children}</div>
