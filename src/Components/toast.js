...
// --- Global dispatcher for static Toast API ---
const __toastGlobal = { add: null, remove: null };
...
// const PLACEMENT_STYLES = {
//   "top-left": "w-fit h-full items-start justify-self-start float-left", // default animation direction -> top-bottom
//   "top-center": "w-fit h-full", // default animation direction -> top-bottom
//   "top-right": "w-fit h-full float-right", // default animation direction -> top-bottom
//   "bottom-left": "w-fit h-full", // default animation direction -> bottom-top
//   "bottom-center": "w-fit h-full", // default animation direction -> bottom-top
//   "bottom-right": "w-fit h-full", // default animation direction -> bottom-top
//   center: "items-center justify-self-center",
// };
// const PLACEMENT_STYLES = {
//   "top-left": "self-start justify-self-start", // default animation direction -> top-bottom
//   "top-center": "self-start justify-self-center", // default animation direction -> top-bottom
//   "top-right": "self-start justify-self-end", // default animation direction -> top-bottom
//   "bottom-left": "self-end justify-self-start", // default animation direction -> bottom-top
//   "bottom-center": "self-end justify-self-center", // default animation direction -> bottom-top
//   "bottom-right": "place-self-end", // default animation direction -> bottom-top
//   center: "items-center justify-self-center",
// };
...
export function ToastProvider({ children }) {
  ...
  // expose dispatcher for static API
  useEffect(() => {
    __toastGlobal.add = addToast;
    __toastGlobal.remove = removeToast;
    return () => {
      __toastGlobal.add = null;
      __toastGlobal.remove = null;
    };
  ...
  return (
    <ToastContext.Provider value={{ addToast, removeToast }}>
      ...
      <div className="fixed inset-0 pointer-events-none z-[9999] overflow-hidden">
        {(<div className="relative w-screen h-screen overflow-auto float-end">
          {toasts.map(({ id, type, message, autoClose, placement, animation, modal }, index) => {
            const fromBottom = typeof placement === 'string' && placement.includes('bottom');
            const styleprop = {
              transform: `translateY(${(fromBottom ? -100 : 100) * (index)}%)`,
            };
            ...
            if (modal) {
              return (
                <div key={id} className="fixed inset-0 z-[10000] pointer-events-auto">
                  ...
                  <ToastWrapper placement={placement} modal styleprop={styleprop}>
                    <ToastView
                      type={type}
                      message={message}
                      autoClose={autoClose}
            ...
            return (
              <ToastWrapper key={id} placement={placement} styleprop={styleprop}>
                <ToastView
                  type={type}
                  message={message}
                  autoClose={autoClose}
...
// --- Static Toast API (no hooks needed) ---
const ensureDispatcher = () => {
  if (typeof window === 'undefined') return false;
  if (!__toastGlobal.add) {
    if (process.env.NODE_ENV !== 'production') {
      console.warn('[Toast] No <ToastProvider/> mounted yet. Call ignored.');
    }
    return false;
  }
  return true;
...
const baseNotify = (type, message, options = {}) => {
  if (!ensureDispatcher()) return null;
  const {
    duration = 4000,
    autoClose = true,
    placement = 'top-right',
  ...
  return __toastGlobal.add({
    id, type, message, duration, autoClose, placement, animation, modal: false,
  });
};

export const Toast = {
  success: (message, options) => baseNotify('success', message, options),
  error:   (message, options) => baseNotify('error', message, options),
  warning: (message, options) => baseNotify('warning', message, options),
  info:    (message, options) => baseNotify('info', message, options),

  /**
   * Promise-style confirm dialog.
   * Usage: const ok = await Toast.promise('Delete item?', { confirmText: 'Delete' });
  ...
  promise: (message, {
    ...
    if (!ensureDispatcher()) return Promise.resolve(false);
    ...
    return new Promise((resolve) => {
      const confirm = () => { resolve(true); __toastGlobal.remove?.(toastId); restore(); };
      const cancel  = () => { resolve(false); __toastGlobal.remove?.(toastId); restore(); };
...
function ToastWrapper({ placement, children, modal = false , className, styleprop}) {
  ...
  return (
    <div
      ...
      style={{ padding: "0.5rem" , ...styleprop}}
    >
      {children}
    </div>
  );
...
function ToastView({ type = "info", message, onClose, autoClose, animation = "top-bottom", modal = false }) {
  ...
  const animClasses = ANIMATION_CLASSES[animation] || ANIMATION_CLASSES["left-right"];
