...
export default function EditCampaignPage() {
  ...
  useEffect(() => {
    (async () => {
      try {
        ...
        toast({ type: 'error', message: 'Failed to load campaign' });
      } finally {
        setLoading(false);
      }
    })();
  ...
  const handleUpdate = async (values) => {
    try {
      ...
      toast({ type: 'success', message: 'Campaign updated' });
      router.push('/manufacturing'); // or go back to list
    } catch (e) {
      toast({ type: 'error', message: e?.response?.data?.message || 'Update failed' });
    } finally {
      setSubmitting(false);
    }
  };
